...
public class Storage {
    ...
    public void init(JavaPlugin plugin) throws SQLException {
        ...
        statement.executeUpdate("CREATE TABLE IF NOT EXISTS cmphantoms_phantom_disabled (uuid VARCHAR(36) PRIMARY KEY)");
    }
    ...
    boolean getPhantomDisabled(UUID uuid) throws SQLException {
        ...
        final String sql = "SELECT EXISTS(SELECT uuid FROM cmphantoms_phantom_disabled WHERE uuid = ?)";
    ...
    void setPhantomDisabled(UUID uuid, boolean isDisabled) throws SQLException {
        ...
        String sql = "DELETE FROM cmphantoms_phantom_disabled WHERE uuid = ?";
        PreparedStatement statement = this.connection.prepareStatement(sql);
        statement.setString(1, uuid.toString());
        statement.executeUpdate();

        if (isDisabled) {
            sql = "INSERT INTO cmphantoms_phantom_disabled (uuid) VALUES (?)";
